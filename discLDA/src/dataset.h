...
public:
    ...
    int * words_cnt;
    string rawstr;
    int length;     // number of unique words
    int totalwdcnt; // number of all words, including repeated
    float rate;     // the rate of the corresponding document
    ...
    words_id = new int[length];
    words_cnt = new int[length];
    rate = 0.0;
    totalwdcnt = 0;
    }
...
public:
    ...
    int Totalwords; // total number of words in corpus
    ...
    // read data with different format as the standard version of the code realease.
    // I use 'nv' to represent 'new version' for short.
    int read_nv_data(string dfile, int dicnum, int docnum, int headernum);
    int read_nv_newdata(string dfile, int dicnum, int docnum, int headernum);
};
